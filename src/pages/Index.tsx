...
const Index = () => {
  ...
  return <div className="min-h-screen bg-black relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        {[...Array(20)].map((_, i) => <div key={i} className="absolute w-1 h-1 bg-pink-500 rounded-full animate-pulse" style={{
      ...
      <div className="relative z-10">
        ...
        {currentView === 'categories' && <CategorySection onSelectCategory={selectCategory} onGoBack={goBack} />}
        {currentView === 'products' && selectedCategory && <ProductGrid category={selectedCategory} onAddToCart={addToCart} onGoBack={goBack} onGoToCart={() => setCurrentView('cart')} cartItemsCount={cartItems.length} />}
        {currentView === 'cart' && <Cart items={cartItems} onRemove={removeFromCart} onCheckout={goToCheckout} onGoBack={goBack} />}
        {currentView === 'checkout' && <Checkout items={cartItems} onGoBack={goBack} onOrderComplete={() => {
        setCartItems([]);
        setCurrentView('hero');
      }} />}
      </div>
      ...
      {/* Cart Notification */}
      {showCart && <div className="fixed top-4 right-4 bg-pink-600 text-white px-6 py-3 rounded-lg shadow-lg animate-slide-in-right z-50">
...
const ProductGrid = ({
  ...
  return <div className="min-h-screen p-8 animate-fade-in">
      ...
      <div className="flex justify-between items-center mb-8">
        <button onClick={onGoBack} className="bg-pink-600 hover:bg-pink-700 text-white px-6 py-3 rounded-lg transition-all duration-300 hover:scale-105">
          ← Back
        </button>
        ...
        <button onClick={onGoToCart} className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg transition-all duration-300 hover:scale-105 relative">
          Cart ({cartItemsCount})
          {cartItemsCount > 0 && <span className="absolute -top-2 -right-2 bg-red-500 text-xs rounded-full w-5 h-5 flex items-center justify-center animate-pulse">
              {cartItemsCount}
            </span>}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {products.map((product, index) => <div key={index} className={`group relative backdrop-blur-sm rounded-xl p-6 border transition-all duration-500 hover:scale-105 hover:shadow-2xl ${product.price === 0 ? 'bg-gradient-to-br from-green-900/20 to-emerald-900/20 border-green-500/20 hover:border-green-400/40 hover:shadow-green-500/20' : 'bg-gradient-to-br from-pink-900/20 to-purple-900/20 border-pink-500/20 hover:border-pink-400/40 hover:shadow-pink-500/20'}`} style={{
        animationDelay: `${index * 100}ms`
      }}>
            {/* FREE Badge */}
            {product.price === 0 && <div className="absolute -top-3 -right-3 bg-green-500 text-white px-3 py-1 rounded-full text-sm font-bold animate-pulse">
                FREE
              </div>}
            
            {/* Preview Image */}
            <div className="relative overflow-hidden rounded-lg mb-4">
              <div className="w-full h-48 bg-cover bg-center transition-transform duration-700 group-hover:scale-110" style={{
            ...
            filter: 'blur(12px) brightness(0.7)'
              ...
              <div className={`absolute top-2 right-2 text-white px-3 py-1 rounded-full text-sm font-semibold ${product.price === 0 ? 'bg-green-600' : 'bg-pink-600'}`}>
                {category === 'videos' ? 'VIDEO' : 'PHOTO'}
              </div>
            </div>

            {/* Product Info */}
            <h3 className={`text-xl font-semibold text-white mb-2 group-hover:transition-colors duration-300 ${product.price === 0 ? 'group-hover:text-green-300' : 'group-hover:text-pink-300'}`}>
              {product.name}
            </h3>
